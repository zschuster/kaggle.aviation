...
# 
# x_train$A = x_train$A[-(sample.int(n = nrow(x_train$A),
#                                    nrow(x_train$A) * .6))]

to_fac_int = c("crew", "seat")
x_train[, (to_fac_int) := lapply(.SD, as.factor),
        .SDcols = to_fac_int]

fac_char = c("event", "experiment")
x_train[, (fac_char) := lapply(.SD, as.factor),
        .SDcols = fac_char]




x_train = DMwR::SMOTE(event ~ .,
                      data = x_train,
                      perc.over = 500,
                      perc.under = 300)
...
x_train[, (to_fac_int) := lapply(.SD, function(x)
  as.integer(as.character(x))),
  .SDcols = to_fac_int]
  
x_train[, (fac_char) := lapply(.SD, as.character),
        .SDcols = fac_char]
